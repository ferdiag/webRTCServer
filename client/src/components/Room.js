...
const Room = () => {
  ...
  const showArray =
    !roomsRef.current || roomsRef.current.length === 0
      ? console.log("now array there")
      : roomsRef.current[indexOfActiveRoom].chat.map((chatInfo, index) => (
          <li
            key={index}
            style={{
              display: "flex",
          ...
          >
            ...
            >
              <div style={{ marginLeft: "5px", fontSize: "10px" }}>
                {chatInfo.nickName} sagte:
              </div>
              <div>{chatInfo.message}</div>
              <div style={{ fontSize: "9px" }}>
                {" "}
                um {chatInfo.timestamp.hours}:{chatInfo.timestamp.minute}
              </div>
              <div style={{ fontSize: "9px" }}>
                {" "}
                am{chatInfo.timestamp.day}:{chatInfo.timestamp.month}:
                {chatInfo.timestamp.year}
              </div>
            </div>
          </li>
        ));
  ...
  return (
    <div
      style={
        roomsRef.current.length > 0 ? { display: "block" } : { display: "none" }
      }
    >
      <NavbarRoom />
      ...
      <input
        ...
        value={inputData.textinput}
        name="textinput"
        style={{ width: "40%", height: "28px", margin: "5px" }}
      />
